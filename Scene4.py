...
class Scene4(GraphScene, Scene):
	...
	def construct(self):
		...
		DTFT = TexMobject(r"X(\omega) = \quad \sum _{ n=-\infty  }^{ \infty  }{ x[n]e^{-j\omega n} }")
		DTFT.set_color(MAROON_A)
		...
		prop_1 = TexMobject(r"\alpha { x }_{ 1 }[n]+\beta { x }_{ 2 }[n]\quad \xrightarrow { DTFT } \quad \alpha { X }_{ 1 }(\omega)+\beta { X }_{ 2 }(\omega)")
		prop_1.set_color(MAROON_A)
		...
		prop_2 = TexMobject(r"x[-n] \quad \xrightarrow { DTFT } \quad X(-\omega)")
		prop_2.set_color(MAROON_A)
		...
		prop_3 = TexMobject(r"Y(\omega )\quad =\quad \sum _{ n=-\infty  }^{ \infty  }{ \sum _{ k=-\infty  }^{ \infty  }{ h[k]x[n-k]{ e }^{ -j\omega n } }  } \\ \qquad \quad \quad put\quad n-k\quad =\quad l\\ \qquad \quad =\quad \left[ \sum _{ l=-\infty  }^{ \infty  }{ x[l]{ e }^{ -j\omega l } }  \right] \left[ \sum _{ k=-\infty  }^{ \infty  }{ h[k]{ e }^{ -j\omega k } }  \right] \\ \qquad \quad =\quad X(\omega )H(\omega )")
		prop_3.set_color(MAROON_A)
		...
		prop_4 = TexMobject(r"\overline { x[n] } \quad \xrightarrow { DTFT } \quad \sum _{ n\quad =\quad -\infty  }^{ \infty  }{ \overline { x[n] } { e }^{ -j\omega n } } \\ \qquad \qquad \qquad =\quad \sum _{ n\quad =\quad -\infty  }^{ \infty  }{ \overline { x[n]{ e }^{ j\omega n } }  } \\ \qquad \qquad \qquad =\quad \overline { \sum _{ n\quad =\quad -\infty  }^{ \infty  }{ x[n]{ e }^{ j\omega n } }  } \\ \qquad \qquad \qquad =\quad \overline { X(-\omega ) } ")
		prop_4.set_color(MAROON_A)
		...
		prop_4_meaning = TexMobject(r"x[n] = \overline{x[n]} \implies X(\omega) = \overline{X(-\omega)}")
		prop_4_meaning.set_color(MAROON_A)
		...
		prop_5 = TexMobject(r"{ x }_{ 1 }[n]{ x }_{ 2 }[n]\quad \xrightarrow { DTFT } \quad \sum _{ n = -\infty  }^{ \infty  }{ { x }_{ 1 }[n]{ x }_{ 2 }[n]{ e }^{ -j\omega n } } \\ \qquad \qquad \qquad \qquad =\quad \quad \sum _{ n = -\infty  }^{ \infty  }{ { x }_{ 1 }[n]",r"\left\{ \frac { 1 }{ 2\pi  } \int _{ -\pi  }^{ \pi  }{ { X }_{ 2 }(\nu ){ e }^{ j\nu n } } d\nu  \right\}",r" { e }^{ -j\omega n } } \\ \qquad \qquad \qquad \qquad =\quad \quad \frac { 1 }{ 2\pi  } \int _{ -\pi  }^{ \pi  }{ { X }_{ 2 }(\nu ) } ",r"\left\{ \sum _{ n = -\infty  }^{ \infty  }{ { x }_{ 1 }[n]{ e }^{ -j(\omega -\nu )n }\quad  }  \right\}",r" d\nu \\ \qquad \qquad \qquad \qquad =\quad \quad \frac { 1 }{ 2\pi  } \int _{ -\pi  }^{ \pi  }{ { X }_{ 2 }(\nu ) } ",r" \left\{ { X }_{ 1 }(\omega -\nu ) \right\}",r" d\nu \\ \qquad \qquad \qquad \qquad =\quad \quad \frac { 1 }{ 2\pi  } { X }_{ 1 }{ *X }_{ 2 }(\omega )")
		prop_5.set_color(MAROON_A)
		prop_5[1].set_color(YELLOW)
		prop_5[3].set_color(YELLOW)
		prop_5[5].set_color(YELLOW)
